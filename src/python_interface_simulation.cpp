...
namespace iebpr
{
	namespace python_interface
	{
		...
		static int EnvStateRecDescr_create(void)
		{
			...
			if (!o)
				return -1;
			assert(PyList_Size(o) == Recorder::EnvStateRecData::arr_size);
		...
		static int AgentStateRecDescr_create(void)
		{
			...
			if (!o)
				return -1;
			assert(PyList_Size(o) == Recorder::AgentStateRecData::arr_size);
		...
		static PyObject *SimulationPyObjectType_method_retrieve_env_state_rec(PyObject *self, PyObject *args)
		{
			...
			Recorder::EnvStateRecData *d_ptr = nullptr;
			// create numpy object
			PyObject *ret = PyArray_Zeros(1, dims, (PyArray_Descr *)EnvStateRecDescr, 0);
			if (!ret)
				goto fail;
			// fill in values
			d_ptr = (Recorder::EnvStateRecData *)PyArray_DATA((PyArrayObject *)ret);
		...
		static PyObject *SimulationPyObjectType_method_retrieve_agent_state_rec(PyObject *self, PyObject *args)
		{
			...
			Recorder::AgentStateRecData *d_ptr;
			// create numpy object
			PyObject *ret = PyArray_Zeros(2, dims, (PyArray_Descr *)AgentStateRecDescr, 0);
			if (!ret)
				goto fail;
			// fill in values
			d_ptr = (Recorder::AgentStateRecData *)PyArray_DATA((PyArrayObject *)ret);
			for (Py_intptr_t i = 0; i < dims[0]; i++)
		...
		static PyObject *SimulationPyObjectType_method_retrieve_snapshot_rec(PyObject *self, PyObject *args)
		{
			...
			for (size_t i = 0; i < vec.size(); i++)
			{
				...
				Recorder::AgentStateRecData *d_ptr = nullptr;
				// create numpy object
				PyObject *t = PyArray_Zeros(2, dims, (PyArray_Descr *)AgentStateRecDescr, 0);
				if (!t)
					goto tuple_build_fail;
				// fill in values
				d_ptr = (Recorder::AgentStateRecData *)PyArray_DATA((PyArrayObject *)t);
